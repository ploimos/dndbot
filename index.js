                    ...
                    isNaN(message.content.split(" ").slice(-1)[0]) == true){
                        ...
                        MongoClient.connect(url, {useNewUrlParser: true, useUnifiedTopology: true}, function (err, db){
                            ...
                            if (!database.collection(col1).find({id: tag})) {
                                ...
                                var oldname;
                                database.collection(col1).find({id: tag}).toArray(function onam(err,res){
                                    oldname = res[0].nome
                                    return oldname
                                })
                                console.log("fuoricoll"+onam(oldname))
                                ...
                                message.reply("Il personaggio che si chiama '"+name+"' ha sovrascritto '"+onam(oldname)+"'.");
                            }
                        })
